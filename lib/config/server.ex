defmodule Arca.Config.Server do
  ...
  def notify_external_change do
    ...
    Logger.debug("Notifying #{length(registry_entries)} callbacks about config change")
    ...
    Registry.dispatch(Arca.Config.CallbackRegistry, :config_change, fn entries ->
      for {process_pid, {id, callback_fn}} <- entries do
        Logger.debug("Calling callback #{inspect(id)} for process #{inspect(process_pid)}")
        # Execute callback in the process that requested it
        try do
          callback_fn.(config)
        rescue
          e -> 
            Logger.error("Config change callback error in #{inspect(id)}: #{inspect(e)}")
        end
      end
    end)
